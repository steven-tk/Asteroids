...
- add pause/menu (esc?)
- add bounce for asteroids and player-on-player (non damage-collision)
- add background
- add acceleration (player) (also missile if added as weapon)
- add life (max 3?) / respawn mechanic
- add drops (+life, fire rate, ..?)
- add explosion effect???
- add screen wrap for player
- add invulnerable frames & dmg to asteroid for ship on asteroid collision)
...
Control via command flags (e.g. -2p) (or menu?)
